...
import { Eye, EyeOff, Lock, Phone, ShieldCheck, MapPin, BellRing, Loader2 } from "lucide-react";
...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Login to track your civic complaints in real time." },
    ],
  }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // The backend returns the real role — use it to route correctly
      toast.success(`Welcome back, ${data.username}!`);
      navigate({ to: dashboardPathForRole(data.role) as never });
    } catch (error: unknown) {
      setErr(true);
      const msg =
  ...
  return (
    <div className="grid min-h-dvh bg-background lg:grid-cols-2">
      ...
      <aside className="relative hidden overflow-hidden bg-gradient-to-br from-primary to-[color-mix(in_oklab,var(--primary)_50%,var(--cat-water))] p-12 text-primary-foreground lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative">
          ...
          <p className="mt-3 max-w-sm text-primary-foreground/80">
            Join 8,400+ citizens shaping Vadodara through transparent civic action.
          </p>
          <ul className="mt-8 space-y-3 text-sm">
            {[
              ...
              <li key={t} className="flex items-center gap-3">
                <span className="inline-flex size-9 items-center justify-center rounded-xl bg-primary-foreground/10">
                  <I className="size-4" />
                </span>
                {t}
              </li>
      ...
      <section className="flex flex-col px-5 py-6 sm:px-12">
        ...
        <div className="ml-auto mt-2 hidden lg:block">
          <ThemeToggle />
        </div>
        ...
        <div className="m-auto w-full max-w-md py-10">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {isStaff ? "Department dashboard awaits." : "Login to track your complaints."}
          </p>
          ...
          <form
            onSubmit={onSubmit}
            className={cn("mt-6 space-y-4", err && "animate-[shake_0.4s_ease-in-out]")}
          >
            <Field
              label={isStaff ? "Username" : "Username / Phone"}
              htmlFor="login-id"
              icon={isStaff ? ShieldCheck : Phone}
            >
              <input
                ref={idRef}
                id="login-id"
                ...
                placeholder={isStaff ? "officer_username" : "your_username"}
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="inline-flex items-center gap-2 text-muted-foreground">
                <input type="checkbox" className="size-4 rounded border" /> Remember me
              </label>
            ...
            <button
              disabled={loading}
              className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary text-sm font-semibold text-primary-foreground shadow-pop transition hover:opacity-90 disabled:opacity-60"
            >
